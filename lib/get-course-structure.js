...
async function getLadokModules (ladokId) {
  const { body } = await ladok.get(`/resultat/kurstillfalle/${ladokId}/moment`)
 ...
 *
 * @param {string} integrationId Integration ID from Canvas
 * @returns {Promise<ExaminationRound>}
 */
async function getLadokExaminationRound (integrationId) {
  const ladokId = integrationId.endsWith('_FUNKA')
    ? integrationId.slice(0, -6)
    : integrationId

  const { body: round } = await ladok.get(
...
function getSectionType (sectionSisId) {
  ...
  const EXAM_ROUND_REGEX = /^AKT\.([a-z0-9-]+)\.(\d\d\d\d-\d\d-\d\d)(_FUNKA)?$/
  ...
  for (const section of sections) {
    if (getSectionType(section.sis_section_id) === 'examination round') {
      allExaminations.push(
        await getLadokExaminationRound(section.integration_id)
      )
